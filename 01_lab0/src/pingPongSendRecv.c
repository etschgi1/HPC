...
int main(int argc, char **argv)
{
    ...
    for (size_t j = 0; j <= MAX_EXPONENT; j++)
    {
        ...
        else if (myRank == 1)
        {
            ...
            for (i=0; i<SAMPLE_COUNT; i++)
            {
                // MPI_Recv(myArray, number_of_elements_received, MPI_INT, 0, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            //     myRank, number_of_elements_to_send);
                ...
                // MPI_Send(myArray, number_of_elements_to_send, MPI_INT, 0, 0, MPI_COMM_WORLD);
