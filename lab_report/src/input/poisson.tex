...
\[t_{\text{no\_improvements}} = \SI{5.66(9)}{\second}\hspace{1cm}\text{and}\hspace{1cm}t_{\text{loop\_improvements}} = \SI{5.63(8)}{\second}\]
So we get a minimal speedup of about 0.5\% by optimizing the loop.\\

\Shining{Why is the speedup relatively small?}\\
We witness the sheer speed of c-loops here. There are certainly multiple factors at play here. Manual optimization of the loops is often not as effective as one might think, because the compiler is already very good at optimizing loops and has probably more tricks up its sleeve than we do. We could test that hypothesis by comparing against versions without any optimization flags (\texttt{-O0}): 
We perform 5 runs with the same setup and \texttt{-O0} and get the following times:
\[t_{\text{no\_improvements}} = \SI{5.664(20)}{\second}\hspace{1cm}\text{and}\hspace{1cm}t_{\text{loop\_improvements}} = \SI{5.69(6)}{\second}\]
